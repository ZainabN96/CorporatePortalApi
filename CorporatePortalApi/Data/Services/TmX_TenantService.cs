...
namespace CorporatePortalApi.Data.Services
{
    public class TmX_TenantService: ITmX_TenantService
    {
        ...
        {
            return await dc.TmX_Tenant.FindAsync(id);
        }

        public async Task<TmX_Tenant> GetWithName(string name)
        {
            return await dc.TmX_Tenant.Where(x => x.Tenant_Name == name)
                                       .FirstOrDefaultAsync();
        }

        public async Task<IEnumerable<TmX_Tenant>> GetTmX_TenantAsync()
        {
            return await dc.TmX_Tenant.Where(x => x.Active_Flag == true)
        ...
        {
            return await dc.TmX_Tenant.AnyAsync(x => x.Tenant_Name == name);
        }

        public async Task<bool> IsTmX_TenantExistInUpdate(string name, int id)
        {
            return await dc.TmX_Tenant.AnyAsync(x => x.Tenant_Name == name && x.Tenant_ID != id);
        }
    }
}
